...
class Logger(BaseProcess):
    ...
    def __init__(
        ...
        Args:
            ...
            logging_queue (mp.Queue): The logging queue where the 
            ``Logger`` will ``get`` to process and save into memory.

            message_to_queue (mp.Queue): The messaging queue used to 
            send messages to the ``Logger``.

            message_from_queue (mp.Queue): The messaging queue used to
